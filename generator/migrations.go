...
func generateMigrations(root string, entities []Entity, opts generatorOptions) (MigrationResult, error) {
	...
	ops = orderMigrationOperations(ops)
	...
	chunks := chunkMigrationOperations(ops)
	files := make([]MigrationFile, 0, len(chunks))
	for _, chunk := range chunks {
		files = append(files, MigrationFile{
			SQL:        renderMigrationSQL(chunk),
			Operations: append([]Operation(nil), chunk...),
		})
	}

	timestamp := opts.now().UTC().Format("20060102150405")
	assignMigrationFilenames(files, opts.MigrationName, timestamp)
	result.Files = files
	result.Snapshot = next
	...
	for i := range files {
		...
		if _, err := writeFile(path, []byte(files[i].SQL)); err != nil {
			return result, fmt.Errorf("write migration %s: %w", files[i].Name, err)
		}
		files[i].Path = path
	}
	if err := writeSchemaSnapshot(root, next); err != nil {
...
func renderMigrationSQL(ops []Operation) string {
	...
	for i, op := range ops {
		buf.WriteString(operationComment(i, op))
...
func orderMigrationOperations(ops []Operation) []Operation {
	if len(ops) <= 1 {
		return ops
	}
	ordered := make([]Operation, 0, len(ops))
	deferred := make([]Operation, 0)
	for _, op := range ops {
		if op.Kind == OpAddForeignKey {
...
func chunkMigrationOperations(ops []Operation) [][]Operation {
	if len(ops) == 0 {
		return nil
	}
	chunks := make([][]Operation, 0)
...
func assignMigrationFilenames(files []MigrationFile, override string, timestamp string) {
	if len(files) == 0 {
		return
	}
	...
	for i := range files {
		slug := normalized
		if slug == "" {
			slug = defaultMigrationSlug(files[i].Operations)
		}
		if len(files) > 1 {
			slug = fmt.Sprintf("%s_%02d", slug, i+1)
		}
		if slug == "" {
			slug = "schema"
		}
		...
		if count := counters[base]; count > 0 {
			slug = fmt.Sprintf("%s_%d", base, count+1)
		...
		files[i].Name = fmt.Sprintf("%s_%s.sql", timestamp, slug)
	}
}

func operationComment(idx int, op Operation) string {
	target := op.Target
	if target == "" {
		target = "global"
	}
	return fmt.Sprintf("-- step %d: %s %s\n", idx+1, op.Kind, target)
}
