...
func TestGeneratorsUsePluralizedNames(t *testing.T) {
	...
	registry := readFile(t, filepath.Join(root, "orm", "gen", "registry_gen.go"))
	...
	client := readFile(t, filepath.Join(root, "orm", "gen", "client_gen.go"))
	...
	schema := readFile(t, filepath.Join(root, "graphql", "schema.graphqls"))
