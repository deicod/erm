...
func writeGraphQLResolvers(root string, entities []Entity, modulePath string) error {
	...
	imports := map[string]struct{}{
		"context": {},
		"fmt":     {},
		fmt.Sprintf("%s/internal/graphql", modulePath):       {},
		fmt.Sprintf("%s/internal/graphql/relay", modulePath): {},
	}
	if len(entities) > 0 {
		imports[fmt.Sprintf("%s/internal/graphql/dataloaders", modulePath)] = struct{}{}
		imports[fmt.Sprintf("%s/internal/orm/gen", modulePath)] = struct{}{}
	}
	if len(imports) > 0 {
...
func writeGraphQLDataloaders(root string, entities []Entity, modulePath string) error {
	...
	imports := map[string]struct{}{
		fmt.Sprintf("%s/internal/observability/metrics", modulePath): {},
		fmt.Sprintf("%s/internal/orm/gen", modulePath):               {},
	}
	if len(entities) > 0 {
