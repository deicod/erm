// Code generated by erm. DO NOT EDIT.
package gen

import (
	"github.com/deicod/erm/internal/orm/dsl"
	"github.com/deicod/erm/internal/orm/runtime"
)

var Registry = runtime.Registry{
	Entities: map[string]runtime.EntitySpec{
		"User": {
			Name:  "User",
			Table: "users",
			Fields: []runtime.FieldSpec{
				{Name: "id", Column: "id", GoType: "string", Type: dsl.TypeUUID, Primary: true, Nullable: false, Unique: false, DefaultNow: false, UpdateNow: false, DefaultExpr: "", Annotations: nil},
				{Name: "created_at", Column: "created_at", GoType: "time.Time", Type: dsl.TypeTime, Primary: false, Nullable: false, Unique: false, DefaultNow: true, UpdateNow: false, DefaultExpr: "", Annotations: nil},
				{Name: "updated_at", Column: "updated_at", GoType: "time.Time", Type: dsl.TypeTime, Primary: false, Nullable: false, Unique: false, DefaultNow: false, UpdateNow: true, DefaultExpr: "", Annotations: nil},
			},
			Edges:   []runtime.EdgeSpec{},
			Indexes: []runtime.IndexSpec{},
		},
	},
}
